...
int main(unsigned long long speid, addr64 argp, addr64 envp) {
...
#ifdef DEBUG 
	printf("Hi, this is SPU #:0x%llx with program spureglerLXYZ\n", speid);
#endif
	...
	while (msg==0) {
		...
		for (i=0; i<4; i++) {
			...
			//VectorControllerVar.arraycontroller.root_vWXYZ[i]=data[i+8];
		}

		MdlOutputsVectorLXYZ(&VectorControllerVar);
		...
		for (i=0; i<4; i++) {
			...
			//data[i+8]=VectorControllerVar.arraycontroller.root_vWXYZ[i];
		}
